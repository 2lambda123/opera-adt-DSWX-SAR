...
def crop_and_save_mgrs_tile_spacing(
    ...
    output_tif_file_path = os.path.join(output_dir_path, output_tif_name)
    ...
    gdal.Warp(output_tif_file_path, source_tif_path, options=warp_options)

    _populate_statics_metadata_datasets(metadata, output_tif_file_path)
...
def find_intersecting_frames_with_bbox(ref_bbox, ref_epsg, input_rtc_files):
    ...
    for input_file in input_rtc_files:
        ...
        with h5py.File(input_file) as src:
            rtc_polygon_str = src["/science/LSAR/identification/boundingPolygon"]
            epsg_code = 4326
            ...
            if epsg_code != ref_epsg:
                ...
                transformer = Transformer.from_crs(
                    f"EPSG:{epsg_code}", f"EPSG:{ref_epsg}", always_xy=True
                )
        ...
        if ref_polygon.intersects(rtc_polygon) or ref_polygon.overlaps(rtc_polygon):
...
def get_intersecting_mgrs_tiles_list_from_db(
    image_tif, mgrs_collection_file, track_number=None
):
    ...
    if antimeridian_crossing_flag:
        ...
        raster_polygon_left = Polygon(
            [(left, bottom), (left, top), (180, top), (180, bottom)]
        )
        raster_polygon_right = Polygon(
            [(-180, bottom), (-180, top), (right, top), (right, bottom)]
        )
        raster_gdf = gpd.GeoDataFrame(
            [1, 2], geometry=[raster_polygon_left, raster_polygon_right], crs=4326
        )
    else:
        # Create a Polygon from the bounds
        ...
        raster_gdf = gpd.GeoDataFrame([1], geometry=[raster_polygon], crs=4326)
    ...
    if track_number is not None:
        vector_gdf = vector_gdf[vector_gdf["track_number"] == track_number].to_crs(
            "EPSG:4326"
        )
    else:
    ...
    intersection = gpd.overlay(raster_gdf, vector_gdf, how="intersection")
...
def run(cfg):
    ...
    if mgrs_db_path is not None and mgrs_collection_db_path is not None:
        logger.info(
            "Both the MGRS tile or the MGRS collection database " "were provided."
        )
        database_bool = True
    else:
        logger.warning(
            "WARNING: Either the MGRS tile or "
    ...
    date_str_list = []
    rtc_reader = mosaic_gcov_frame.RTCReader(row_blk_size=200, col_blk_size=200)
    ...
    date_str_id = datetime.datetime.strptime(
        date_str_id_temp, input_date_format
    ...
    if inundated_vege_cfg.enabled == "auto":
        ...
        total_inundated_vege_flag = inundated_vege_cfg.enabled
    ...
    if merge_layer_flag:
        ...
        if merge_layer_flag:
            ...
            merge_pol_list = ["_".join(pol_set1), "_".join(pol_set2)]
    else:
        pol_set1 = pol_list
        pol_set2 = []
        count_pols = []
        for input_dir in input_list:
            pol_types = DSWX_NI_POL_DICT["DV_POL"] + DSWX_NI_POL_DICT["DH_POL"]
            # Initialize count
            count_pol = 0
            # Count files for each polarization type
            for target_pol in pol_types:
                pol_files = glob.glob(os.path.join(input_dir, f"*{target_pol}*.tif"))
                count_pol += len(pol_files)
            count_pols.append(count_pol)
        all_match_first = all(count == count_pols[0] for count in count_pols)
    ...
    for key, prefix in prefix_dict.items():
        ...
        if merge_layer_flag:
            list_layers = [
                os.path.join(scratch_dir, f"{prefix}_{pol_cand_str}.tif")
            ...
            if not inundated_vege_mosaic_flag and key in [
                "inundated_veg",
                "inundated_veg_target",
                ...
                merge_pol_layers(
                    list_layers, os.path.join(scratch_dir, file_path), **extra_args
                )
    ...
    layover_shadow_mask_path = os.path.join(
        scratch_dir, "mosaic_layovershadow_mask.tif"
    )

    if os.path.exists(layover_shadow_mask_path):
        layover_shadow_mask = dswx_sar_util.read_geotiff(layover_shadow_mask_path)
    ...
    full_wtr_water_set_path = os.path.join(scratch_dir, "full_water_binary_WTR_set.tif")
    full_bwtr_water_set_path = os.path.join(
        scratch_dir, "full_water_binary_BWTR_set.tif"
    )
    full_conf_water_set_path = os.path.join(
        scratch_dir, "full_water_binary_CONF_set.tif"
    )
    full_diag_water_set_path = os.path.join(
        scratch_dir, "full_water_binary_DIAG_set.tif"
    )

    # 4) inundated_vegetation
    if total_inundated_vege_flag:
        inundated_vegetation = dswx_sar_util.read_geotiff(paths["inundated_veg"])
        ...
        inundated_vegetation_mask = (inundated_vegetation == 2) & (water_map == 1)
    ...
    if dswx_workflow == "opera_dswx_ni":
        ...
        region_grow_map = dswx_sar_util.read_geotiff(paths["region_growing"])
        landcover_map = dswx_sar_util.read_geotiff(paths["landcover_mask"])
    ...
    if partial_water_flag:
        ...
        temp_full_wtr_water_set_path = os.path.join(
            scratch_dir, "full_water_binary_WTR_set_temp.tif"
        )
    ...
    if database_bool:
        ...
        actual_frame_id = collect_burst_id(input_list, DSWX_NI_POL_DICT["CO_POL"])
        number_frame = len(actual_frame_id)
        mgrs_meta_dict["MGRS_COLLECTION_EXPECTED_NUMBER_OF_BURSTS"] = maximum_frame
        mgrs_meta_dict["MGRS_COLLECTION_ACTUAL_NUMBER_OF_BURSTS"] = number_frame
        missing_frame = len(list(set(expected_frame_list) - set(actual_frame_id)))
        mgrs_meta_dict["MGRS_COLLECTION_MISSING_NUMBER_OF_BURSTS"] = missing_frame
    ...
    if dswx_workflow == "opera_dswx_ni":
        ...
        for mgrs_num_id, mgrs_tile_id in enumerate(unique_mgrs_tile_list):
            ...
            if mgrs_db_path is None:
                ...
                (x_value_min, x_value_max, y_value_min, y_value_max, epsg_output) = (
                    get_bounding_box_from_mgrs_tile_db(mgrs_tile_id, mgrs_db_path)
                )
            mgrs_bbox = [x_value_min, y_value_min, x_value_max, y_value_max]
            overlapped_burst = find_intersecting_frames_with_bbox(
                ref_bbox=mgrs_bbox, ref_epsg=epsg_output, input_rtc_files=input_list
            )
            ...
            if overlapped_burst:
                ...
                dswx_name_format_prefix = (
                    f"OPERA_L3_DSWx-NI_T{mgrs_tile_id}_"
    ...
    logger.info("successfully ran save_mgrs_tiles in " f"{t_all_elapsed:.3f} seconds")
