...
def find_intersecting_frames_with_bbox(ref_bbox, ref_epsg, input_rtc_files):
    ...
    ref_polygon = Polygon([(minx, miny), (minx, maxy), (maxx, maxy),
                           (maxx, miny)])
    ...
    for input_file in input_rtc_files:
        ...
        if ref_polygon.intersects(rtc_polygon) or ref_polygon.overlaps(
                rtc_polygon):
...
def get_intersecting_mgrs_tiles_list_from_db(image_tif,
    ...
    if antimeridian_crossing_flag:
        ...
        raster_polygon_left = Polygon([(left, bottom), (left, top), (180, top),
                                       (180, bottom)])
        raster_polygon_right = Polygon([(-180, bottom), (-180, top),
                                        (right, top), (right, bottom)])
        raster_gdf = gpd.GeoDataFrame(
            [1, 2],
            geometry=[raster_polygon_left, raster_polygon_right],
            crs=4326)
    else:
        # Create a Polygon from the bounds
        raster_polygon = Polygon([(left, bottom), (left, top), (right, top),
                                  (right, bottom)])
    ...
    if track_number is not None:
        vector_gdf = vector_gdf[vector_gdf["track_number"] ==
...
def run(cfg):
    ...
    date_str_id = datetime.datetime.strptime(
        date_str_id_temp, input_date_format).strftime(output_date_format)
    ...
    if pol_options is not None and merge_layer_flag:
        ...
        merge_pol_list = [
            item + "_" + pol_option_str for item in merge_pol_list
        ]

    # Depending on the workflow, the final product are different.
    prefix_dict = {
        "final_water":
        ("bimodality_output_binary" if dswx_workflow == "opera_dswx_ni" else
         "region_growing_output_binary"),
        "landcover_mask":
        "refine_landcover_binary",
        "no_data_area":
        "no_data_area",
        "region_growing":
        "region_growing_output_binary",
    ...
    no_data_raster = (no_data_raster
                      > 0) | (water_map == band_assign_value_dict["no_data"])

    # 2) layover/shadow
    layover_shadow_mask_path = os.path.join(scratch_dir,
                                            "mosaic_layovershadow_mask.tif")

    if os.path.exists(layover_shadow_mask_path):
        layover_shadow_mask = dswx_sar_util.read_geotiff(
    ...
    full_wtr_water_set_path = os.path.join(scratch_dir,
                                           "full_water_binary_WTR_set.tif")
    full_bwtr_water_set_path = os.path.join(scratch_dir,
                                            "full_water_binary_BWTR_set.tif")
    full_conf_water_set_path = os.path.join(scratch_dir,
                                            "full_water_binary_CONF_set.tif")
    full_diag_water_set_path = os.path.join(scratch_dir,
                                            "full_water_binary_DIAG_set.tif")
    ...
    if total_inundated_vege_flag:
        ...
        inundated_vegetation_mask = (inundated_vegetation == 2) & (water_map
                                                                   == 1)
        ...
        if iv_target_file_type == "auto":
            ...
            interp_glad_path_str = os.path.join(scratch_dir,
    ...
    if database_bool:
        ...
        mgrs_meta_dict[
            "MGRS_COLLECTION_EXPECTED_NUMBER_OF_BURSTS"] = maximum_frame
        mgrs_meta_dict[
            "MGRS_COLLECTION_ACTUAL_NUMBER_OF_BURSTS"] = number_frame
        missing_frame = len(
            list(set(expected_frame_list) - set(actual_frame_id)))
        mgrs_meta_dict[
            "MGRS_COLLECTION_MISSING_NUMBER_OF_BURSTS"] = missing_frame
        ...
        for mgrs_num_id, mgrs_tile_id in enumerate(unique_mgrs_tile_list):
            ...
            if mgrs_db_path is None:
                (x_value_min, x_value_max, y_value_min, y_value_max,
                 epsg_output) = (get_bounding_box_from_mgrs_tile(mgrs_tile_id))
            else:
                (x_value_min, x_value_max, y_value_min, y_value_max,
                 epsg_output) = (get_bounding_box_from_mgrs_tile_db(
                     mgrs_tile_id, mgrs_db_path))
