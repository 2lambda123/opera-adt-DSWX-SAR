...
from dswx_sar import pre_processing
...
def run(cfg):
    ...
    filtered_image_path = os.path.join(scratch_dir, filtered_images_str)
    ...
    for anc_type, anc_filename in relocated_ancillary_filename_set.items():
        ...
        ancillary_path = Path(os.path.join(scratch_dir, anc_filename))
    ...
    if processing_cfg.debug_mode:
        ...
        if filtered_intensity.ndim == 2:
            filtered_intensity = np.expand_dims(filtered_intensity, axis=0)
...
def main():
    ...
    if pol_mode == "MIX_DUAL_POL":
        proc_pol_set = [DSWX_NI_POL_DICT["DV_POL"], DSWX_NI_POL_DICT["DH_POL"]]
    elif pol_mode == "MIX_SINGLE_POL":
        proc_pol_set = [DSWX_NI_POL_DICT["SV_POL"], DSWX_NI_POL_DICT["SH_POL"]]
    else:
